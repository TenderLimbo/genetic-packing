...
def get_fittest_solution(solutions):
    ...
    for solution in min_area_solutions:
        ...
        if placing_density < min_placing_density:
            ...
            min_placing_density_solutions = [solution]
            min_placing_density = placing_density

        elif placing_density == min_placing_density:

            min_placing_density_solutions.append(solution)

        solution.packing_density = placing_density


    if not min_placing_density_solutions:
        return random.choice(min_area_solutions)
    else:

        return min_placing_density_solutions[0]
    ...
    # for solution in min_bound_area_solutions:
    #
...
def mutate_with_addition(solution, max_attempt_num):
    ...
    if feasible_item_indices:
        ...
        for _ in range(max_attempt_num):
            ...
            if solution.add_item(item_index, (random.uniform(min_x, max_x), random.uniform(min_y, max_y)), random.choice([90, 180])):
                return True
    ...
    if solution.placed_items:
        ...
        for _ in range(max_attempt_num):
            ...
            if action_index == position_change_index:
                ...
                has_modified = solution.rotate_item_to(item_index, random.choice([90, 180]))
                ...
                has_modified = solution.move_and_rotate_item_to(item_index, (random.uniform(min_x, max_x), random.uniform(min_y, max_y)), random.choice([90, 180]))
